...
const uni = [];
for (i = 0; i < graduates.length; i++) {
  let uppercaseUni = graduates[i].university.toUpperCase();
  if (uppercaseUni.includes('UNI')) {
...
The zoo wants to display both the scientific name and the animal name in front of the habitats.  Return an array with only the animal 
// and scientific names in it.  The individual values in the array should look like this "Name: Jackal, asiatic, Scientific: Canis aureus."
...
const lowerCase = zooAnimals.map(function(ele) {  
  return ele.animal_name.toLowerCase();
})
...
const largerPopulation2 = zooAnimals.filter(function (animal) {
  return animal.population < 5;
})
console.log(largerPopulation2[0]);
console.log(largerPopulation2[1]);
console.log(largerPopulation2[2]);
